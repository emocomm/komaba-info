(function () {
  ...
  const publicKeyExercise = (() => {
    ...
    function bytesToBase64Url(bytes) {
      ...
      for (let index = 0; index < bytes.length; index += chunkSize) {
        binary += String.fromCharCode.apply(
          null,
          bytes.slice(index, index + chunkSize),
        );
      }
      return btoa(binary)
        .replace(/\+/g, "-")
    ...
    async function verifySignature(target, signatureText, publicKey) {
      ...
      const verified =
        signaturePayload.targetHash === targetHash &&
    ...
    function initKeygen() {
      ...
      document.querySelector("#pk-generate").addEventListener("click", () => {
        if (savedKeypair()) {
          const ok = confirm(
            "鍵を作り直すと、前の鍵で作った署名は新しい公開鍵では検証できません。作り直しますか？",
          );
          if (!ok) return;
        }
        createPair();
      ...
      toggle.addEventListener("click", () => {
        ...
        toggle.textContent = secret.classList.contains("hidden")
          ? "秘密鍵を表示する"
          : "秘密鍵を隠す";
      });

      document
        .querySelector("#pk-copy-secret")
        .addEventListener("click", () => {
          const pair = savedKeypair();
          if (!pair) return;
          const ok = confirm(
            "秘密鍵は他の人に共有しないものです。本当にコピーしますか？",
          );
          if (ok) copyText(pair.secretKey);
        });

      document
        .querySelector("#pk-download-public")
        .addEventListener("click", () => {
          const pair = savedKeypair();
          if (!pair) return;
          downloadText(
      ...
      document
        .querySelector("#pk-download-secret")
        .addEventListener("click", () => {
          const pair = savedKeypair();
          if (!pair) return;
          const ok = confirm(
            "秘密鍵は他の人に共有しないものです。本当にダウンロードしますか？",
          );
          if (ok)
            downloadText(
              pair.secretKey,
    ...
    function initEncryptTool() {
      ...
      run.addEventListener("click", async () => {
        ...
        if (!publicKey.startsWith("PUB-")) {
          alert(
            "確認してください。暗号化に使う鍵欄には PUB-... から始まる公開鍵を入れてください。",
          );
          return;
        }
        ...
        if (!confirmed.checked) {
          alert(
            "確認してください。UTOLで共有された指紋と、ツールに表示された指紋・見た目を見比べてからチェックを入れてください。",
          );
          return;
        }
        try {
          ...
          storageSet(
            keys.encryptConfirmed,
            confirmed.checked ? "true" : "false",
          );
          ...
          setJson(keys.teacherFingerprint, {
    ...
    function initSignTool() {
      ...
      bindStoredValue(
        target,
        keys.signTarget,
        savedCipher && savedCipher.cipherText,
      );
      ...
      run.addEventListener("click", async () => {
        ...
        if (!secretKey.startsWith("SEC-")) {
          alert(
            "確認してください。署名に使う鍵欄には SEC-... から始まる秘密鍵を入れてください。",
          );
          return;
        }
        let secretPayload;
        ...
        if (
          teacher &&
          teacher.fingerprint &&
          secretPayload.publicFingerprint === teacher.fingerprint
        ) {
          alert(
        ...
        try {
          ...
          const targetPreview =
            targetText.slice(0, 180) + (targetText.length > 180 ? "..." : "");
    ...
    function buildSubmissionText() {
      ...
      const signature = document
        .querySelector("#submission-signature")
        .value.trim();
      const publicKey = document
        .querySelector("#submission-public-key")
        .value.trim();
      return [
        "暗号文:",
        cipher,
    ...
    function initSubmission() {
      ...
      button.addEventListener("click", () => {
        ...
        if (/SEC-/.test(text)) {
          alert(
            "確認してください。提出テキストに SEC-... から始まる秘密鍵が含まれています。秘密鍵は提出しません。",
          );
        } else {
    ...
    function parseSubmission(text) {
      return {
        cipherText: extractBetween(text, "暗号文", [
          "ディジタル署名",
          "署名",
          "自分の公開鍵",
          "公開鍵",
        ]),
        signature: extractBetween(text, "ディジタル署名", [
          "自分の公開鍵",
          "公開鍵",
        ]),
    ...
    function answerChecks(text) {
      const checks = [
        ...
        [
          "E",
          "一方向ハッシュ関数",
          countOccurrences(text, "一方向ハッシュ関数") >= 2,
        ],
        ["F", "電子指紋", countOccurrences(text, "電子指紋") >= 3],
        [
          "G",
          "秘密鍵",
          /送信者の秘密鍵/.test(text) && /秘密鍵の所有者/.test(text),
        ],
    ...
    function renderAnswerChecks(text) {
      ...
      if (!container) return;
      const rows = answerChecks(text)
        .map(([label, expected, ok]) => {
          return (
            "<tr><td>" +
            label +
            "</td><td>" +
        ...
        .join("");
      container.innerHTML =
        "<table><thead><tr><th>空欄</th><th>想定語</th><th>判定</th></tr></thead><tbody>" +
        rows +
        "</tbody></table>";
    }
    ...
    function initTeacher() {
      ...
      button.addEventListener("click", async () => {
        const submission = parseSubmission(
          document.querySelector("#teacher-submission-input").value,
        );
        const teacherSecret = compact(
          document.querySelector("#teacher-secret-key").value,
        );
        const diagnostics = [];
        if (
          ...
          setCallout(
            "#teacher-parse-result",
            "提出テキストから暗号文、ディジタル署名、自分の公開鍵の3項目を読み取れませんでした。",
            true,
          );
          return;
        }
        ...
        if (
          document
            .querySelector("#teacher-submission-input")
            .value.includes("SEC-")
        ) {
          diagnostics.push(
            "提出テキストに SEC-... が含まれています。秘密鍵を提出している可能性があります。",
          );
        }
        ...
        try {
          ...
          setText(
            "#teacher-decrypt-meta",
            "復号に失敗しました。教員の秘密鍵か暗号文を確認してください。",
          );
          setValue("#teacher-plaintext", "");
          diagnostics.push(
            "暗号文を教員の秘密鍵で復号できませんでした。教員の公開鍵以外で暗号化した可能性があります。",
          );
        }
        ...
        try {
          ...
          setCallout(
            ...
            verify.verified
              ? "署名は検証できました。"
              : "署名を検証できませんでした。暗号文、署名、公開鍵の組み合わせを確認してください。",
            !verify.verified,
          );
          if (!verify.verified)
            diagnostics.push(
              "署名検証に失敗しています。署名対象や公開鍵が違う可能性があります。",
            );
        } catch (error) {
          setCallout(
            "#teacher-signature-result",
            "署名検証に失敗しました。SIG-... または PUB-... を確認してください。",
            true,
          );
        ...
        try {
          ...
          if (cipherPayload.recipientFingerprint === studentFp) {
            diagnostics.push(
              "暗号文の宛先が学生本人の公開鍵になっています。教員宛の暗号化になっていない可能性があります。",
            );
          }
          if (
            decrypted &&
            decrypted.secretPayload.publicFingerprint !==
              cipherPayload.recipientFingerprint
          ) {
            diagnostics.push(
              "暗号文の宛先指紋と、復号に使った秘密鍵の対応公開鍵指紋が一致していません。",
            );
          }
        } catch (error) {
